...
# 这个文件的作用
# 因为模型已经按照之前的处理，可以将零件聚集为板单元，
# 聚集的结果为occ-id编号形成的对应表，一个板单元包含几个至几十个零件的occ-id
# 当然之前也是可以提出ifc-id聚集成板单元的形式。
# 根据板单元对应的零件occ-id，可以将整个节段的模型进行拆分，
# 一个板单元单独一个模型，
# 这个文件的主要作用就是将整个节段模型拆分成独立的板单元模型
